...
class InputBox(Widget):
    def init(self, title,x,y,width,height):
        super().__init__(x,y)
        self.text=""
    ...
    def render(self):
        ...
        rectangle(self.app.stdscr,self.x,self.y,self.x+self.size[0],self.y+self.size[1])
        self.app.stdscr.addstr(self.y,self.x,self.title)
    def handle_input(self,key):
        if not self.done:
            if key in typeable_chars:
